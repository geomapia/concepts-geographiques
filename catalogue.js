      ...
      return `
        <article class="special-notice${item.niveau_conceptuel === "Fondamental" ? " fundamental-notice" : ""}" id="${slug(item.concept)}">
          ...
          <div class="notice-actions">
            ...
            ${item.type_notice === "Concept géographique" ? `<a class="relations-link" href="./relations-explorer.html?concept=${encodeURIComponent(item.concept)}" target="_blank" rel="noopener">Explorer les relations ↗</a>` : ""}
...
function openModal(item, updateUrl = true) {
  ...
  if (modalRelations) {
    modalRelations.href = `./relations-explorer.html?concept=${encodeURIComponent(item.concept)}`;
    modalRelations.target = "_blank";
    modalRelations.rel = "noopener";
  }
